...
forward FireStationaryArtillery(base);
forward MoveBomb(base, id);
forward FireBomb(base, id);
forward RenewTowers();
...
public OnFilterScriptInit()
{
        ...
        print("X        Artillery filterscript  V0.9       X");
		print("X           created by Mauzen 2009          X");
...
public OnPlayerCommandText(playerid, cmdtext[])
{
		...
		if(strcmp(cmd, "/addbase", true) == 0) {
			if(IsPlayerAdmin(playerid) ) {
			    GetPlayerPos(playerid, sx[basecount], sy[basecount], sz[basecount]);
			    AddBase(sx[basecount], sy[basecount], sz[basecount] - 0.5);
			    SetPlayerPos(playerid, sx[basecount - 1], sy[basecount - 1], sz[basecount - 1] + 5);
			    new tmp[64];
			    format(tmp, 64, "Added a new base! ID: %d", basecount - 1);
		...
		if(strcmp(cmd, "/setrotation", true) == 0) {
			...
			if(arotator[base] == -1) {
				...
				format(tmp, 32, "rotaiong base %d to %f", base, wa);
...
public Follower(playerid, base, target) {
		GetPlayerPos(target, tx[base], ty[base], tz[base]);
		AutoAim(base, playerid, 500, 1, tx[base], ty[base], tz[base]);
		return 1;
}
...
public FireStationaryArtillery(base) {
	...
	while((id < MAX_BOMBS) && (bombcount[base] < firecount[base])) {
		if(bomb[base][id] == 0) {
			...
			bombcount[base] ++;
			SetTimerEx("FireBomb", delay[base] * bombcount[base] + 100, 0, "ii", base, id);
		}
		id ++;
	}
}
...
public MoveBomb(base, id) {
	...
	for(new i = 0; i < MAX_PLAYERS; i++) {
	    if(GetPlayerDistanceToPoint(i, curx[base][id], cury[base][id], curz[base][id]) < 5) {
		    ...
		    CreateExplosion(curx[base][id], cury[base][id], curz[base][id], 6, 5.0);
...
public AddBase(Float:x, Float:y, Float:z) {
	if(basecount < MAX_BASES){
        atower[basecount] = CreateObject(3267, x, y, z, 0, 0, 0);
        abase[basecount] = CreateObject(3277, x, y, z, 0, 0, 0);
        arotator[basecount] = -1;
        apower[basecount] = 25;
        aangle[basecount] = 45;
        firecount[basecount] = 1;
        streuung[basecount] = 2;
        sx[basecount] = x;
        sy[basecount] = y;
        sz[basecount] = z;
        delay[basecount] = 500;
