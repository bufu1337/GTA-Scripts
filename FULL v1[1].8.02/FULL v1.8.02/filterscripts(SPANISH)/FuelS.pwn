...
public OnFilterScriptInit()
	...
	AddMenuItem(MenuGas,0,"Tanque LLENO");
	AddMenuItem(MenuGas,0,"MEDIO tanque");
	    ...
	    if 	(GetPlayerState(playerid)==PLAYER_STATE_DRIVER)
        	...
        	format(tmp, sizeof(tmp), "Fuel:%d", combustible[GetPlayerVehicleID(playerid)]);
         	if	(combustible[GetPlayerVehicleID(playerid)]<1)
                {
                format(tmp, sizeof(tmp), "VACIO", combustible[GetPlayerVehicleID(playerid)]);
        ...
        if ((IsPlayerConnected(player))&&(IsPlayerInAnyVehicle(player)))
            ...
            if ((type==1)|(type==2)|(type==3))
                ...
                if (combustible[wid]<1)
                    {
                    format(string, sizeof(string), "VACIO");
                    if 	(GetPlayerState(player)==PLAYER_STATE_DRIVER)
                    	{
						SinGas[player]=wid;
	    ...
	    switch (row)
	        ...
	        case 0:	{
					if (combustible[vid] == FULLTANK)
						{
						SendClientMessage(playerid, COLOR_RED, "El vehiculo ya tiene el tanque lleno.");
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Le has puesto %d litros a tu vehiculo y ahora esta lleno", cant);
					format(string2, sizeof(string2), "Le han puesto %d litros al vehiculo y ahora esta lleno", cant);
			...
			case 1: {
					if (combustible[vid] >= FULLTANK/2)
						{
						SendClientMessage(playerid, COLOR_RED, "El vehiculo ya tiene medio tanque o mas.");
                        TogglePlayerControllable(playerid,true);
						return 1;
						}
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Le has puesto %d litros a tu vehiculo y ahora tiene medio tanque", cant);
					format(string2, sizeof(string2), "Le han puesto %d litros al vehiculo y ahora tiene medio tanque", cant);
					SendClientMessage(playerid, COLOR_YELLOW, string);
					format(string, sizeof(string), "Fuel:%d", combustible[vid]);
					actualizarMedidor(playerid,string);
					for (new j=0;j<MAX_PLAYERS;j++)
  ...
  MessageForDriver(playerid, vehicleid)
	...
	format(string, sizeof(string), " ");
 	new type=GetVehicleTypeFromId(GetVehicleModel(vehicleid));
  	if 	((type==1)|(type==2)|(type==3))
	    {
		SendClientMessage(playerid, COLOR_WHITE, "Para cargarle combustible, busca una estacion de servicio");
    ...
    if 	(CommandsSTATUS)
		...
		if	(strcmp(cmd, "/buygas", true) == 0)
			...
			if	(!IsCloserThanThis(playerid,20.0))
			    {
			    SendClientMessage(playerid, COLOR_RED, "Debes estar en una estacion de servicio.");
			    return 1;
			    }
			if (!IsPlayerInAnyVehicle(playerid))
			    {
			    SendClientMessage(playerid, COLOR_RED, "Puede ser pasajero o conductor, pero debes estar en un auto para usar el comando");
			    return 1;
			    }
		  	new vid =GetPlayerVehicleID(playerid);
			if (combustible[vid] == FULLTANK)
				{
				SendClientMessage(playerid, COLOR_RED, "Este auto ya tiene el tanque lleno");
				return 1;
				}
	        tmp = strtok(cmdtext, idx);
			if(!strlen(tmp))
				{
				SendClientMessage(playerid, COLOR_WHITE, "USO: /buygas [litros]");
				return 1;
				}
	        new cant = strval(tmp);
	        if ( (cant<1) || (cant>1024))
	            {
	            SendClientMessage(playerid, COLOR_RED, "Ingresa una cantidad valida");
	            return 1;
	            }
			if (PRECIOLITRO*cant>GetPlayerMoney(playerid))
			    {
			...
			new string[255];
			new string2[255];
			format(string, sizeof(string), "Les has puesto %d litros", cant);
			format(string2, sizeof(string2), "Le han puesto &d litros al auto, ahora el tanque esta lleno", cant);
			SendClientMessage(playerid, COLOR_YELLOW, string);
			format(string, sizeof(string), "Fuel:%d", combustible[vid]);
			actualizarMedidor(playerid,string);
			for (new j=0;j<MAX_PLAYERS;j++)
