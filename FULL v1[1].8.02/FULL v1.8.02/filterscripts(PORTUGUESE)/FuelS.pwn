...
public OnFilterScriptInit()
	...
	AddMenuItem(MenuGas,0,"Tanque Cheio");
	AddMenuItem(MenuGas,0,"Meio Tanque");
	AddMenuItem(MenuGas,0,"Não Abastecer");
	    ...
	    if 	(GetPlayerState(playerid)==PLAYER_STATE_DRIVER)
        	...
        	format(tmp, sizeof(tmp), "Fuel:%d", combustible[GetPlayerVehicleID(playerid)]);
         	if	(combustible[GetPlayerVehicleID(playerid)]<1)
                {
                format(tmp, sizeof(tmp), "Vazio", combustible[GetPlayerVehicleID(playerid)]);
	    ...
	    switch (row)
	        ...
	        case 0:	{
					if (combustible[vid] == FULLTANK)
						{
						SendClientMessage(playerid, COLOR_RED, "O Tanque está cheio.");
					...
					if (PRECIOLITRO*cant>GetPlayerMoney(playerid))
					    {
			            SendClientMessage(playerid, COLOR_RED, "Você não tem grana,pode pegar no barco.");
			            TogglePlayerControllable(playerid,true);
			            return 1;
					    }
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Você comprou %d litros de gasolina.", cant);
					format(string2, sizeof(string2), "Foram colocados %d litros. O tanque está cheio.", cant);
					SendClientMessage(playerid, COLOR_YELLOW, string);
					format(string, sizeof(string), "Gasolina:%d", combustible[vid]);
			...
			case 1: {
					if (combustible[vid] >= FULLTANK/2)
						{
						SendClientMessage(playerid, COLOR_RED, "O Tanque está meio cheio.");
                        TogglePlayerControllable(playerid,true);
						return 1;
						}
					...
					if (PRECIOLITRO*cant>GetPlayerMoney(playerid))
					    {
			            SendClientMessage(playerid, COLOR_RED, "Você não tem dinheiro para isso.");
                        TogglePlayerControllable(playerid,true);
						return 1;
					    }
			        combustible[vid]=combustible[vid]+cant;
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Você comprou %d litros de gasolina, o tanque está cheio.", cant);
					format(string2, sizeof(string2), "O Seu carro foi abastecido com %d litros, tanque cheio doutor!", cant);
					SendClientMessage(playerid, COLOR_YELLOW, string);
					format(string, sizeof(string), "Gasolina:%d", combustible[vid]);
					actualizarMedidor(playerid,string);
					for (new j=0;j<MAX_PLAYERS;j++)
  ...
  MessageForDriver(playerid, vehicleid)
	...
	format(string, sizeof(string), " ");
 	new type=GetVehicleTypeFromId(GetVehicleModel(vehicleid));
  	if 	((type==1)|(type==2)|(type==3))
	    {
		SendClientMessage(playerid, COLOR_WHITE, "Para abastecer, visite um posto de gasolina.");
		format(string, sizeof(string), "Fuel:%d", combustible[vehicleid]);
    	}
	if 	(combustible[vehicleid]<1)
	    {
	    format(string, sizeof(string), "Vaziu");
...
//------------------------------------------------------------------------------------------------------
  MessageForPassenger(playerid,vehicleid)
	...
	new string[255];
 	...
 	if 	((type==1)|(type==2)|(type==3))
 	    {
		format(string, sizeof(string), "Gasolina:%d", combustible[vehicleid]);
    	}
	if (combustible[vehicleid]<1)
	    {
	    format(string, sizeof(string), "Vaziu");
