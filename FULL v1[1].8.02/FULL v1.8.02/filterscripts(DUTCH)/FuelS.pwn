	    ...
	    if 	(GetPlayerState(playerid)==PLAYER_STATE_DRIVER)
        	...
        	format(tmp, sizeof(tmp), "Fuel:%d", combustible[GetPlayerVehicleID(playerid)]);
         	if	(combustible[GetPlayerVehicleID(playerid)]<1)
                {
                format(tmp, sizeof(tmp), "LEEG", combustible[GetPlayerVehicleID(playerid)]);
        ...
        if ((IsPlayerConnected(player))&&(IsPlayerInAnyVehicle(player)))
            ...
            if ((type==1)|(type==2)|(type==3))
                ...
                if (combustible[wid]<1)
                    {
                    format(string, sizeof(string), "LEEG");
                    if 	(GetPlayerState(player)==PLAYER_STATE_DRIVER)
                    	{
						SinGas[player]=wid;
	    ...
	    switch (row)
	        ...
	        case 0:	{
					if (combustible[vid] == FULLTANK)
						{
						SendClientMessage(playerid, COLOR_RED, "Deze auto is alweer vol.");
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Je hebt %d liter benzine gekocht.", cant);
					format(string2, sizeof(string2), "Je auto is weer vol met %d liter benzine. De tank is vol.", cant);
					SendClientMessage(playerid, COLOR_YELLOW, string);
					format(string, sizeof(string), "Benzine:%d", combustible[vid]);
			...
			case 1: {
					if (combustible[vid] >= FULLTANK/2)
						{
						SendClientMessage(playerid, COLOR_RED, "De tank is halfvol.");
                        TogglePlayerControllable(playerid,true);
						return 1;
						}
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Je kocht %d liter benzine. De tank is vol", cant);
					format(string2, sizeof(string2), "Je auto is weer bijgevuld met %d liter benzine. De tank is vol.", cant);
					SendClientMessage(playerid, COLOR_YELLOW, string);
  ...
  MessageForDriver(playerid, vehicleid)
	...
	format(string, sizeof(string), " ");
 	new type=GetVehicleTypeFromId(GetVehicleModel(vehicleid));
  	if 	((type==1)|(type==2)|(type==3))
	    {
		SendClientMessage(playerid, COLOR_WHITE, "Rijd naar een tankstation om te tanken.");
		format(string, sizeof(string), "Benzine:%d", combustible[vehicleid]);
    	}
