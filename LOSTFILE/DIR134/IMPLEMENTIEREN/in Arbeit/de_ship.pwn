...
de_ship - Counter-Strike in San Andreas
...
#define BOMBPLACE -1291.0239,501.6888,11.1953 // x, y and z coordinates for Bombplace Checkpoint
...
{
	print("\n---------------------------------------");
	print("  de_ship - Counter-Strike in San Andreas");
...
public OnGameModeInit()
{
	...
	// Counter-Terrorists
	AddPlayerClass(285,-1314.5493,503.5343,18.2344,95.3573,4,0,24,42,29,360); // CT-Spawn

	// Terrorists
	AddPlayerClass(108,-1447.1777,501.0267,3.0414,269.1892,4,0,22,102,30,360); // Terror-Spawn
	return 1;
}
...
public SetupPlayerForClassSelection(playerid)
{
	...
	SetPlayerPos(playerid,-1389.7386,659.9529,84.0078);
    SetPlayerCameraPos(playerid,-1391.0818,670.9420,92.5133);
	SetPlayerCameraLookAt(playerid,-1389.7386,659.9529,84.0078);
...
{
	SendClientMessage(playerid, COLOR_YELLOW, "de_ship - Counter-Strike in San Andreas - by Eightball");
...
public OnPlayerSpawn(playerid)
{
	...
	switch (gPlayerClass[playerid]) {
	    case 0:
	        {
				GameTextForPlayer(playerid, "~g~GO GO GO~n~~b~Defend the Ship!", 4000, 3);
				SetPlayerColor(playerid,COLOR_BLUE);
				if(create_t_cp == 0) SetPlayerCheckpoint(playerid,BOMBPLACE,2.0);
			}
	    case 1:
	        {
				GameTextForPlayer(playerid, "~g~GO GO GO~n~~r~Blow up the Ship!", 4000, 3);
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	{
	    SendClientMessage(playerid, COLOR_YELLOW, "The Terrorists have to plant a bomb in the ship and defend it until it blows up.");
		SendClientMessage(playerid, COLOR_YELLOW, "The Counter-Terrorists have to defend the ship and defuse the bomb if it got planted.");
...
public TerrorsWin() {
	...
	switch (secs_to_blow) {
	    ...
	    case 10: format(output, sizeof(output), "~r~The bomb will blow up in %d seconds!", secs_to_blow);
	    case 20: format(output, sizeof(output), "~w~The bomb will blow up in %d seconds!", secs_to_blow);
	    case 30: format(output, sizeof(output), "~w~The bomb will blow up in %d seconds!", secs_to_blow);
	    case 60: format(output, sizeof(output), "~w~The bomb will blow up in %d seconds!", secs_to_blow);
	    case 120: format(output, sizeof(output), "~w~The bomb will blow up in %d seconds!", secs_to_blow);
	    }
...
//---------------------------------------------------------
public DamDefended() {
    SendClientMessageToAll(COLOR_YELLOW, "*** Counter-Terrorists successfully defended the ship!");
    EndTheRound(CT_WIN);
    return;
    }
