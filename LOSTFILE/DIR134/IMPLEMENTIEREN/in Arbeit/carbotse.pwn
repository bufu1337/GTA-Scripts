...
public OnFilterScriptInit(){
	for(new i=0;i<MAX_DRIVEPOINTS;i++) for(new j=0;j<4;j++) if(Connections[i][j] != 0) ConnectionsAmount[i]++;
...
public ObjectUpdate(){
	for(new i=0; i<MAX_PLAYERS; i++)
	{
		if(IsPlayerConnected(i))
		{
			...
			GetXYInFrontOfObject(i,x3,y3,5.0);
			new Vi;
			...
			{
				GetObjectPos(AutomaticCars[j],x,y,z);
				if(GetPointDistanceToPoint(x,y,x3,y3) < 15 && (floatabs(BotAngle[j] - vang) < 50 || floatabs(BotAngle[j] - vang) > 310) && !PositionsWechsel(i) && IsPlayerInAnyVehicle(i))
				{
					if(!HasFreezed[i][j])
					{
				    	StopObject(AutomaticCars[j]);
...
Float:GetXYInFrontOfObject(objectid, &Float:x, &Float:y, Float:distance){
	new Float:a;
	new Float:xx,Float:yy;
	GetObjectPos(AutomaticCars[objectid], x, y, a);
	GetObjectRot(AutomaticCars[objectid],xx,yy,a);
