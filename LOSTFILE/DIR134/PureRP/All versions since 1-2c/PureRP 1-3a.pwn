...
public Payday()
{
new newstr[225],player[24],interest,bank[255],amount;
new Float:balance;
...
balance = floatstr(bank);
interest = floatround(balance * 0.04,floatround_round);
switch (gTeam[p]){
	case TEAM_CIVS:{
...
if (Servertime==24) Servertime=0;
	if (Servertime<10) format(newstr,sizeof(newstr),"0%d:00h",Servertime);
...
{
	SetGameModeText("Pure RPG v.1.3a"); //Added by M1k3
	ShowNameTags(1);
	ShowPlayerMarkers(1);
...
AddStaticVehicleEx(522,-1953.0000,296.0000,41.0000,134.0000,19,19,3300000);// 178 NRG500 Wang Cars
...
dcmd_commands(playerid,params[])
{
	...
	SendClientMessage(playerid,COLOR_ORANGE,"/job | /pay | /rob | /L[ocal chat] | /saveskin");
	...
	SendClientMessage(playerid,COLOR_RED,"Other: /fuel | /flightcommands | /minigames | /cophelp | /ah");
...
format(string,sizeof(string),"[server] Admin %s respawned all 178 vehicles.",player);
print (string);
WriteEcho(string);
SendClientMessageToAll(COLOR_YELLOW,string);
