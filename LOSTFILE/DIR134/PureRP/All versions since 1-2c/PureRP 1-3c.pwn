...
pInfo[p][UsedDrugs] = 0;
TogglePlayerClock(p,Servertime);
...
{
	SetGameModeText("Pure RPG v.1.3c"); //Added by M1k3
	ShowNameTags(1);
	ShowPlayerMarkers(1);
...
AddStaticVehicleEx(538,1701.0000,-1954.0000,14.0000,90.0000,0,1,300000);// 180 Train Unity Station
...
tmp = dini_Get(udb_encode(player),"Banned");
	...
	SendClientMessage(playerid,COLOR_ORANGE,"Type /laws to read the server rules.");
	...
	TogglePlayerClock(playerid,Servertime);
    if(hack > 5)
    {
    	hack = 0;
        Ban(playerid);
...
if (pInfo[playerid][Jailed]){
	SendClientMessage(playerid,COLOR_RED,"You are not allowed to chat in public while jailed/muted/badge loss!");
 	return 0;
	}
...
{
	dcmd(help,4,cmdtext);
	dcmd(laws,4,cmdtext); // Rules edited to laws by M1k3
...
// The following commands have been added by M1k3
	...
	dcmd(jail,4,cmdtext);
	dcmd(sue,3,cmdtext);
	dcmd(guilty,6,cmdtext);
	dcmd(lawyer,6,cmdtext);
	dcmd(bail,4,cmdtext);
	dcmd(wanted,6,cmdtext);
	dcmd(cophelp,7,cmdtext);
	dcmd(courthelp,9,cmdtext);
	dcmd(ah,2,cmdtext);
	dcmd(sell,4,cmdtext);
	dcmd(find,4,cmdtext);
...
{
	new player[24],str[86],tmp1[255],tmp2[255],tmp3[255],jobcheck;
...
case 181: format(tmp2,sizeof(tmp2),"Train (Location: SF Train Station)");
}
...
dcmd_commands(playerid,params[])
{
	...
	SendClientMessage(playerid,COLOR_ORANGE,"/taxi  | /R | /buy | /exit | /sell | /find | /showhouse | /sue");
	SendClientMessage(playerid,COLOR_ORANGE,"/bail | /deposit | /balance | /drugs | /buydrugs | /usedrugs | /selldrugs");
	SendClientMessage(playerid,COLOR_RED,"Other: /fuel | /bizhelp | /flightcommands | /minigames | /cophelp | /courthelp | /ah");
...
strresult1 = strfind(params,"y",true,0);
strresult2 = strfind(params,"n",true,0);
...
//Function added by M1k3
dcmd_courthelp(playerid,params[])
{
	#pragma unused params
...
dcmd_ah(playerid,params[])
{
    ...
    SendClientMessage(playerid,COLOR_WHITE,"[2] /(un)mute [ID] - allows/disallows a player to speak in public and local chat.");
...
//Function added by M1k3
dcmd_lawyer(playerid,params[])
{
#pragma unused params

new player[24],jobcheck;
...
{
#pragma unused params
new player[24],blockID,tmp[255],blockstr[120];
...
format(string,sizeof(string),"[server] Admin %s respawned & unlocked all 181 vehicles.",player);
print (string);
WriteEcho(string);
SendClientMessageToAll(COLOR_YELLOW,string);
