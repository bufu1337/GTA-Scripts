	    ...
	    if 	(GetPlayerState(playerid)==PLAYER_STATE_DRIVER)
        	...
        	format(tmp, sizeof(tmp), "Fuel:%d", combustible[GetPlayerVehicleID(playerid)]);
         	if	(combustible[GetPlayerVehicleID(playerid)]<1)
                {
                format(tmp, sizeof(tmp), "LEER", combustible[GetPlayerVehicleID(playerid)]);
	    ...
	    switch (row)
	        ...
	        case 0:	{
					if (combustible[vid] == FULLTANK)
						{
						SendClientMessage(playerid, COLOR_RED, "Das Fahrzeug ist bereits aufgetankt.");
					...
					if (PRECIOLITRO*cant>GetPlayerMoney(playerid))
					    {
			            SendClientMessage(playerid, COLOR_RED, "Ohne Moos, nix los^^.");
			            TogglePlayerControllable(playerid,true);
			            return 1;
					    }
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Du hast %d getankt, und der Tank ist voll.", cant);
					format(string2, sizeof(string2), "Dein Auto wurde mit %d Litern betankt, der Tank ist voll.", cant);
			...
			case 1: {
					if (combustible[vid] >= FULLTANK/2)
						{
						SendClientMessage(playerid, COLOR_RED, "Der Tank ist mindestens halbvoll.");
                        TogglePlayerControllable(playerid,true);
						return 1;
						}
					...
					new string[255];
					new string2[255];
					format(string, sizeof(string), "Du hast %d getankt, der Tanks ist voll", cant);
					format(string2, sizeof(string2), "Dein Auto wurde mit %d Litern betankt, der Tank ist voll", cant);
					SendClientMessage(playerid, COLOR_YELLOW, string);
					format(string, sizeof(string), "Fuel:%d", combustible[vid]);
					actualizarMedidor(playerid,string);
