...
{
	SetGameModeText("Movie Making Filming Gamemode");
	ShowNameTags(1);
	ShowPlayerMarkers(1);
	...
	SetTimer("VehicleResetWarning",1700000,1);
	SetTimer("VehicleReset",1800000,1);
...
public OnPlayerSpawn(playerid)
{
	...
	if(IsLoggedIn[playerid])
	{
	if(TutorialPassed[playerid] == 0)
	{
		SetTimerEx("STut2", 700, 0, "p", playerid);
...
public OnPlayerCommandText(playerid,cmdtext[])
{
			...
			if(Admin[playerid] > 0)
		    {
				if(IsPlayerConnected(giveplayerid))
				{
					...
					else
					{
						...
						format(string, sizeof(string), "[Admin]: %s has been kicked by %s. [reason: %s]", giveplayer, sendername, result);
		...
		if (IsLoggedIn[playerid])
	    {
			if(Admin[playerid] > 1)
		    {
				if(IsPlayerConnected(giveplayerid))
				{
					...
					{
						GetPlayerName(giveplayerid, giveplayer, sizeof(giveplayer));
						format(string, sizeof(string), "[Admin]: %s has been banned by %s. [reason: %s]", giveplayer, sendername, result);
		...
		if (IsLoggedIn[playerid])
	    {
			if(Admin[playerid] > 1)
		    {
				if(IsPlayerConnected(giveplayerid))
				{
					...
					{
						GetPlayerName(giveplayerid, giveplayer, sizeof(giveplayer));
						format(string, sizeof(string), "[Admin]: %s has been sent to a tutorial by %s. [reason: %s]", giveplayer, sendername, result);
						SendClientMessageToAll(COLOR_PURPLE, string);
						TutorialPassed[giveplayerid] = 0;
						dUserSet(PlayerName(giveplayerid)).("Tutorial", "0");
						SendClientMessage(giveplayerid, COLOR_LIGHTBLUE, "* You have been automaticly kicked by the xGBot. Please reconnect. *");
						Kick(giveplayerid);
					}
				}
				else
			...
			if (IsPlayerConnected(plo))
			{
			    if(plo != INVALID_PLAYER_ID)
			    {
					if (Admin[playerid] >= 1)
					{
						...
						if (GetPlayerState(plo) == 2)
						{
							SendClientMessage(playerid, COLOR_YELLOW, "You slapped somebody out of their vehicle.");
							new Float:slx, Float:sly, Float:slz;
							GetPlayerHealth(plo, health);
							SetPlayerHealth(plo, health-5);
							GetPlayerPos(plo, plocx, plocy, plocz);
							SetPlayerPos(plo, plocx, plocy, plocz+5);
							GetPlayerPos(giveplayerid, slx, sly, slz);
                    		PlayerPlaySound(plo, 1130, slx, sly, slz+5);
                    		GetPlayerName(plo, giveplayer, sizeof(giveplayer));
							format(string, sizeof(string), "[Admin]: %s has been slapped out of their vehicle by %s.", giveplayer, sendername);
							SendClientMessageToAll(COLOR_PURPLE, string);
						}
						else
						{
                    		...
                    		PlayerPlaySound(plo, 1130, slx, sly, slz+5);
                    		GetPlayerName(plo, giveplayer, sizeof(giveplayer));
							format(string, sizeof(string), "[Admin]: %s has been slapped by %s.", giveplayer, sendername);
							SendClientMessageToAll(COLOR_PURPLE, string);
						}
					}
					else
	...
	if(strcmp(cmd,"/vclear",true) == 0)
	{
		...
		if(IsLoggedIn[playerid] == 1)
		{
			if(Admin[playerid] > 2)
		    {
					format(string, sizeof(string), "[Admin]: %s has set all previously used vehicles to clear in 10 seconds.", sendername);
					SendClientMessageToAll(COLOR_PURPLE, string);
	...
	if(strcmp(cmd,"/all2me",true) == 0)
	{
		...
		if(IsLoggedIn[playerid] == 1)
		{
			if(Admin[playerid] > 1)
		    {
					format(string, sizeof(string), "[Admin]: %s has teleported everyone.", sendername);
					SendClientMessageToAll(COLOR_PURPLE, string);
	...
	{
		SendClientMessage(playerid, COLOR_YELLOW2,"*** ACCOUNT *** /register - /login - /logout - /rules");
		SendClientMessage(playerid, COLOR_YELLOW2,"*** GENERAL *** /jetpack - /film - /v(vehicle) - /w2(weapon) - /t(time) - /s(skin)");
		SendClientMessage(playerid, COLOR_YELLOW2,"*** GENERAL *** /animhelp - /teles - /kill - /letterbox(on/off) - /names(on/off)");
		SendClientMessage(playerid, COLOR_YELLOW2,"*** VEHICLE *** /vc(vehicle chat) - /fix");
		SendClientMessage(playerid, COLOR_YELLOW2,"*** NAME *** /nick");
		if(Admin[playerid] == 1)
		{
			SendClientMessage(playerid, COLOR_YELLOW, "*** LEVEL 1 *** /a(adminchat) - /gethere - /goto - /kick - /reaction");
		}
		if(Admin[playerid] == 2)
		{
			SendClientMessage(playerid, COLOR_YELLOW, "*** LEVEL 2 *** /a(adminchat) - /gethere - /goto - /ban - /kick - /learn - /all2me - /reaction - /slap");
		}
		if(Admin[playerid] == 3)
		{
			SendClientMessage(playerid, COLOR_YELLOW, "*** LEVEL 3 *** /a(adminchat) - /gethere - /goto - /ban - /kick - /vclear - /learn - /all2me - /reaction - /slap");
		}
		if(Admin[playerid] == 4)
		{
			SendClientMessage(playerid, COLOR_YELLOW, "*** LEVEL 4 *** /a(adminchat) - /gethere - /goto - /ban - /kick - /vclear - /learn - /all2me - /reaction - /slap");
		}
		if(Admin[playerid] == 5)
		{
			SendClientMessage(playerid, COLOR_YELLOW, "*** LEVEL 5 *** /a(adminchat) - /gmx - /gethere - /goto - /ban - /kick - /vclear - /learn - /all2me - /reaction - /slap");
		}
		return 1;
	}
	...
	{
		SendClientMessage(playerid, COLOR_YELLOW, "____-Rules of the gamemode/server-____");
		...
		SendClientMessage(playerid, COLOR_LIGHTBLUE, "6. Do not deathmatch unless in a secret empty place with your friends only.");
		SendClientMessage(playerid, COLOR_LIGHTBLUE, "7. Do not ask for admin or how to be one.");
		return 1;
	}
	...
	{
  		SendClientMessage(playerid, COLOR_YELLOW2, "____-Teleports-____");
	...
	{
		SendClientMessage(playerid, COLOR_YELLOW2, "____-Animations-____");
		...
		SendClientMessage(playerid, COLOR_YELLOW,"/deal - /crack - /smoke - /groundsit - /chat - /fucku - /strip - /hide - /vomit - /eat - /chairsit - /reload");
...
{
	GameTextForPlayer(playerid, "~w~-Tutorial ~n~~g~Part 1/5", 1000, 1);
	PlayerPlaySound(playerid,1052,0,0,0);
	SendClientMessage(playerid, COLOR_YELLOW, "____-Tutorial Part One: So You've Been Naughty-____");
	SendClientMessage(playerid, COLOR_YELLOW2, "First of all, why the fuck did you get sent to a tutorial.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Don't mess about. These things happen if you don't do what your told.");
	SendClientMessage(playerid, COLOR_YELLOW2, "Here's a few lessons on how to BEHAVE you fudgelicker nub.");
...
public STut3(playerid)
{
	...
	PlayerPlaySound(playerid,1052,0,0,0);
	SendClientMessage(playerid, COLOR_YELLOW, "____-Tutorial Part Two: Lesson One-____");
	SendClientMessage(playerid, COLOR_YELLOW2, "First of all you've either disobeyed a rule that is commonly known.");
	SendClientMessage(playerid, COLOR_YELLOW2, "Or an admin's been a fag and just put you here. Eitherway you should always obey the rules.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Failure to do so will end you up here like a lil' nubfag.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "Don't interupt people either, or deathmatch using the weapon or vehicle system. Even if you manage");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "to bug it and use it anytime you want.");
	SendClientMessage(playerid, COLOR_YELLOW2, "Here is some rules:");
	SetTimerEx("STut4", 21300, 0, "j", playerid);
	return 1;
}
...
public STut4(playerid)
{
	...
	PlayerPlaySound(playerid,1052,0,0,0);
	SendClientMessage(playerid, COLOR_YELLOW, "____-Server Tutorial Part Three: Rules____");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "1. Do not Hack/Cheat. It's lame and will get you banned. (We do have anticheat).");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "2. Do not use mods which effect the gameplay for you. (Skin mods are ok).");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "3. Do not use map mods as you could be banned for suspected hacking.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "4. Talk properly and none of this gay shit: ''WTF DNT KYLL MEH PLZKTHNXBAI''. Lame, stupid. Will get you temp banned.");
...
public STut5(playerid)
{
	...
	PlayerPlaySound(playerid,1052,0,0,0);
	SendClientMessage(playerid, COLOR_YELLOW, "____-Server Tutorial Part Four: Lecture____");
	SendClientMessage(playerid, COLOR_YELLOW2, "Once again, you obviously failed to follow one of them rules.");
	SendClientMessage(playerid, COLOR_YELLOW2, "Now i'm giving you one more chance. Go out there and follow the rules.");
	SendClientMessage(playerid, COLOR_LIGHTBLUE, "You can view them again with /rules, at anytime.");
...
public STut6(playerid)
{
	...
	PlayerPlaySound(playerid,1052,0,0,0);
	SendClientMessage(playerid, COLOR_YELLOW, "____-Tutorial Part Five: Cya____");
	SendClientMessage(playerid, COLOR_YELLOW2, "I don't want to see you back here again. Cya.");
