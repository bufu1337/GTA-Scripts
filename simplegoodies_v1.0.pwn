...
if(strcmp(cmdtext, "/sghelp", true) == 0) {
		...
		SendClientMessage(playerid, 0xFFFF00AA,"Type /vehiclehealth to enable or disable the vehicle-health-bar.");
		SendClientMessage(playerid, 0xFFFF00AA,"Type /sgcredits to read the credits for SimpleGoodies.");
		return 1;
	}

if(strcmp(cmdtext, "/sgcredits", true) == 0) {
		SendClientMessage(playerid, 0x33AA33AA,"SimpleGoodies Credits");
		SendClientMessage(playerid, 0x33CCFFAA,"SimpleGoodies is created by: Mr-Tape");
...
SendClientMessage(i,0xFF9900AA,"This server is running SimpleGoodies by: Mr-Tape.");
SendClientMessage(i,0xFF9900AA,"Type /sghelp for more info.");
announce[i] = 1;
