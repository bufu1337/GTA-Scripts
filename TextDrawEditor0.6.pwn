...
UpdateTextDraw(curr,destroy)
{
	...
	if(destroy)
		...
		format(stringg,sizeof(stringg), "TextX: %f TextY: %f Text: %s",	textx[curr],texty[curr],textdrawtext[curr]);
	...
	if(box[curr])
		...
		format(stringg,sizeof(stringg), "boxX: %f boxY: %f", boxsizex[curr], boxsizey[curr]);
		SendClientMessageToAll(COLOR_RED, stringg);
		}
	...
	format(stringg,sizeof(stringg), "String: %s Align: %d Font: %d LetterX: %f LetterY: %f Outline: %d",textdrawtext[curr],alignment[curr],font[curr],lettersizex[curr],lettersizey[curr],outline[curr]);
	SendClientMessageToAll(COLOR_RED, stringg);
}
